...
class VisualRobot(Robot):
    ...
    def visual_move(self, target, Kp, Ki, Kd, thres, jac=True):
        ...
        while rospy.Time.now() - t0 < rospy.Duration(5):
            ...
            try:
                ...
                s1 = self.cart_from_matrix(matrix_from_StampedTransform(msg)) + [0, 0, 0.0, 0, 0]
                error = s1 - self.cart()

                if jac == True:
                    
                    dev = np.linalg.norm(error)

                    if pasterror is None:
                        pasterror = error
                    deriv = (error - pasterror)/self.dt
                    pasterror = error
                    integ = integ + error*self.dt
                    if dev > thres:
                        integ = 0
                        deriv = 0

                    ds = Kp*error + Ki*integ + Kd*deriv
                    ...
                    # alfa = speed * self.dt * (rospy.Time.now() - t0).to_sec()**1.7
                    # self.q = self.q + dq * alfa
    ...
    def scan(self, target, sweep=np.arange(0, math.pi/2, 5e-2)):

        sweep = np.append(sweep, np.flipud(sweep))
        sweep = np.append(sweep, -sweep)
        # center
        self.move([0.05*math.cos(sweep[0]), 0.05*math.sin(sweep[0]), 0.2, sweep[0], 0])

        dq = self.dt*math.pi/0.3
        eps = math.pi/12
    ...
    def connect(self):
        ...
        self.move(self.home)
        self.scan('fconn')
        self.visual_move('fconn_', 0.03, 0.01, 0.1, 0)
        self.visual_move('fconn', 0.03, 0.01, 0.1, 0)
        pub.publish(0)
        rospy.sleep(3)
        self.move(self.home)
